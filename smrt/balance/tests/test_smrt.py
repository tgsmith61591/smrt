...
def test_smrt_mnist():
    ...
    X_smrt, y_smrt, encoders = smrt_balance(X, y, n_hidden=450, n_latent_factors=10, learning_rate=0.05,
                                            activation_function='sigmoid', verbose=2, display_step=25,
                                            n_epochs=50, batch_size=256, random_state=42, shuffle=False,
                                            balance_ratio=0.5, return_estimators=True)
    ...
    X_smrt, y_smrt = smrt_balance(X, y, n_hidden=450, n_latent_factors=10, learning_rate=0.05,
                                  activation_function='sigmoid', verbose=2, display_step=25,
                                  n_epochs=50, batch_size=256, random_state=42, shuffle=False,
                                  balance_ratio=0.5, return_estimators=False, prefit_estimators=encoders)
